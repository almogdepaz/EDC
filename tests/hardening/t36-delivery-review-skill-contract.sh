...
check "delivery architecture reference verifies Octocode candidates conservatively" "$(file_has "$ARCHITECTURE_AXIS" 'OCTOCODE_STATUS: available' && file_has "$ARCHITECTURE_AXIS" "callers, public contracts, integration completeness, ownership evidence, and installed dependency behavior" && file_has "$ARCHITECTURE_AXIS" "lexical search, exact reads, and the smallest runnable verification" && file_has "$ARCHITECTURE_AXIS" "cannot prove dead code or zero references" && echo 1 || echo 0)"
