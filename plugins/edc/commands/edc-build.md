...
- **If `.meta.json` does NOT exist** (or `--force`) → run full build + split + audit:
  1. Invoke the `edc:edc-context` skill (NOT `audit-context-building` — that is a different plugin) for the full workflow. Write the complete analysis to `.context/full-context.md`.
  2. Then run `/edc:edc-split` to produce `.context/context.md` + `.context/{module}.md` + `.context/issues.md` + `.context/.meta.json`.
