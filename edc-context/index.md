...
|---|---|---|---|
| `plugins/edc/scripts/`, `plugins/edc/commands/`, `install.sh`, CLI modes, Bash/version/model/spawn behavior, watchdogs/process termination, optional source-research capability, worker pools, build/update/review/security/delivery/quality orchestrators, staged promotion, result JSON | [runtime-cli](modules/runtime-cli.md) | canonical-skills; plugin-surface; agent-wrappers; hardening-tests | Shell/Node coordinators own runtime preflight, routing, capability detection, recovery, task graphs, worker concurrency/cancellation, staging, validation, canonical promotion, structured results, and backend spawning. |
| `README.md`, `package.json`, `.claude-plugin/`, `plugins/edc/hooks/`, `plugins/edc/README.md`, package contents, Claude/Cursor injection, managed runtime inventory, shared termination timing | [plugin-surface](modules/plugin-surface.md) | runtime-cli; agent-wrappers; canonical-skills; hardening-tests | JS routing/freshness/injection mirrors shell semantics, feeds Pi, distributes worker helpers/shared policies, transactionally verifies runtime artifacts, and defines package/docs contracts. |
...
| `pi/`, Pi menu/session/tool-call/status/background jobs, kill/cancellation UX, extracted argument/scope/result-display helpers, Pi install docs/media, Pi worker observer docs | [agent-wrappers](modules/agent-wrappers.md) | plugin-surface; runtime-cli; canonical-skills; hardening-tests | Pi imports shared JS helpers/policies and runs shell orchestrators as background jobs; it verifies termination before changing status but must not fork shell routing or worker semantics. |
...
- Shell/Node coordinators own build-vs-update decisions, immutable review-candidate resolution, context recovery, review scope normalization, optional source-research capability detection, task manifests, worker concurrency, timeout/cancellation, staged validation, canonical promotion, parallel review-all phase aggregation, structured result/status files, and deterministic validation.
...
- `agent-wrappers` depends on `plugin-surface` for Pi injection/freshness helpers and termination timing, and on `runtime-cli` for `.edc/scripts/edc-*.sh` jobs. Its scope/default-base/kill UX affects args and status around review/update/delivery/quality, but not orchestrator routing.
