...
description: >
  ...
  blast radius, checks test coverage, leverages .context/ files when available,
...
1. **Output-First**: Create the output file (`issues.md` or `.context/reports/issues.md`) BEFORE any analysis begins. Append findings as discovered. Never batch to end.
...
**EDC context files (`.context/`):**
- If `.context/index.md` exists, load it for architecture overview and module map
- Map changed files to modules, load relevant `.context/modules/{module}.md`
- Load `.context/reports/issues.md` to check if changes touch known issues
...
- Change touches code flagged in `.context/reports/issues.md`
