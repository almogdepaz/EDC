...
For each `.context/modules/{module}.md`:
...
Use the `.context/modules/{module}.md` cross-module coupling sections to identify where the analysis already flagged duplication.
...
For key entrypoints documented in `.context/index.md`, trace the call chain from entrypoint to actual work. Count the number of function hops. Flag chains where:
...
Write two report files under `.context/reports/` (create the directory if it does not exist). Reports MUST live under `.context/reports/` — never at the top level of `.context/`.
...
Promote any audit findings that represent real correctness, security, or operational risks (as opposed to pure bloat) into `.context/reports/issues.md`. If the file already exists from a prior `edc-build-impl` run, merge new findings in rather than overwriting unrelated entries. Use this structure:
