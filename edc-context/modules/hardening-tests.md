...
- `t14-resolve-prompt-decoupled.sh`: prompt resolution from a trusted package/global root, independent of dev and repo-local paths. It verifies complete audit references and Octocode behavior for working/failing/hanging/absent CLIs, accepted and rejected repository roots, unified structural/evidence/continuation/graph guidance, one immutable capability decision per process, and rejection of ordinary caller-preseeded capability state.
...
- Optional research tests must cover available, broken, hanging, absent, repository-path-rejected, and caller-preseeded states while proving one immutable coordinator capability decision, review-all child inheritance, schema-conditional evidence metadata, explicit empty/error semantics, partial-result continuations, no installation/hard dependency, immediate native fallback, assigned-scope limits, uncertainty, and manifest authority.
