...
check "build prompt delegates process lifecycle exclusively to coordinator" \
  "$(contains "$BUILD_PROMPT" "shell coordinator owns" && contains "$BUILD_PROMPT" "Never launch an agent" && not_contains "$BUILD_PROMPT" "spawn ONE subagent" && echo 1 || echo 0)"
...
check "update prompt uses worker-owned atomic manifest-only replacement" \
  "$(contains "$UPDATE_PROMPT" "**worker** owns this manifest-file-only promotion" && contains "$UPDATE_PROMPT" ".manifest-inputs/partial-manifest.XXXXXX" && contains "$UPDATE_PROMPT" "CLI ARGUMENTS (JSON argv)" && contains "$UPDATE_PROMPT" 'PARTIAL_MANIFEST=<exact-prepared-partial-path>' && contains "$UPDATE_PROMPT" 'bash "$EDC_SCRIPTS_DIR/edc-manifest.sh"' && contains "$UPDATE_PROMPT" "manifest_ignore_args=()" && contains "$UPDATE_PROMPT" "trap 'exit 143' TERM" && contains "$UPDATE_PROMPT" 'mv "$STAGED_MANIFEST" "$MANIFEST"' && contains "$UPDATE_PROMPT" "prior manifest remains" && contains "$UPDATE_PROMPT" "does **not** make the whole update transactional" && echo 1 || echo 0)"
check "update prompt uses coordinator-approved Octocode evidence without replacing routing authority" \
  "$(contains "$UPDATE_PROMPT" 'OCTOCODE_STATUS: available' && contains "$UPDATE_PROMPT" "changed public or high-risk symbols" && contains "$UPDATE_PROMPT" "definitions, references, callers, and importers" && contains "$UPDATE_PROMPT" "additional blast-radius evidence" && contains "$UPDATE_PROMPT" "not proof of no references or dead code" && contains "$UPDATE_PROMPT" "smallest runnable check" && contains "$UPDATE_PROMPT" "Route every discovered repository path through the shared classifier" && contains "$UPDATE_PROMPT" "manifest routing remains authoritative" && echo 1 || echo 0)"
...
check "module output requirements preserve staged authority without scratch quotas" \
  "$(contains "$MODULE_OUTPUT_REQUIREMENTS" "coordinator-declared staged path" && contains "$MODULE_OUTPUT_REQUIREMENTS" "worker never writes canonical context directly" && contains "$MODULE_OUTPUT_REQUIREMENTS" "omit copied constants, inventories, obvious function narration, and private scratch structure" && contains "$MODULE_OUTPUT_REQUIREMENTS" "edc-context/modules/<name>.md" && not_contains "$MODULE_OUTPUT_REQUIREMENTS" "Minimum 3" && not_contains "$MODULE_OUTPUT_REQUIREMENTS" "Minimum 5" && echo 1 || echo 0)"
check "module prompt derives coverage rather than reasoning quotas" \
  "$(contains "$MODULE_PROMPT" "Coverage is evidence-based, not a quota" && contains "$MODULE_PROMPT" "only when one resolves a concrete uncertainty" && not_contains "$MODULE_PROMPT" "Apply **First Principles**, **5 Whys**, and **5 Hows**" && not_contains "$MODULE_PROMPT" "Apply micro-level First Principles, 5 Whys, and 5 Hows" && not_contains "$MODULE_PROMPT" "Minimum 3 invariants" && not_contains "$MODULE_PROMPT" "Minimum 5 assumptions" && echo 1 || echo 0)"
