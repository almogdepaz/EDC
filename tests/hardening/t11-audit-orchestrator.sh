...
if [[ "\$prompt" == *"edc-build"* ]]; then
  mkdir -p .context/modules .context/reports
...
setup_repo() {
  ...
  if [ "$with_context" = "fresh" ]; then
    ...
    mkdir -p .context/modules
    printf '<!-- t11 -->\n# Stub\n\n## Module Map\n\n- root\n' > .context/index.md
    printf '{"schemaVersion":2,"sourceCommit":"deadbeef","modules":[{"name":"root","doc":".context/modules/root.md"}]}\n' > .context/manifest.json
    printf '<!-- t11 -->\n# root\n\n## Files\n\n- src.py\n' > .context/modules/root.md
  fi
}
...
if [ "$result" -eq 0 ] && [ -f .context/reports/complexity.md ] && [ -f .context/reports/issues.md ]; then
