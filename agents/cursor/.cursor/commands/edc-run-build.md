...
**Arguments:** optional `--force` to rebuild from scratch, `--focus <module>` for specific module

## Routing

Check if `.context/.meta.json` exists AND `--force` was NOT passed:

- **If `.meta.json` exists** → follow the `edc-run-update` command (incremental update based on branch changes)
- **If `.meta.json` does NOT exist** (or `--force`) → run full build + split + audit:
  ...
  2. Then follow the `edc-run-split` command to produce `.context/context.md` + `.context/{module}.md` + `.context/issues.md` + `.context/.meta.json`.
  3. Then follow the `edc-run-audit` command to produce `.context/complexity.md`.
